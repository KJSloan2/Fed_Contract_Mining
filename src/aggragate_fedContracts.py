...
#####################################################################################
#####################################################################################
gaz_zcta = get_gaz(["GEOID","INTPTLAT","INTPTLONG"],"2021_Gaz_cbsa_national.txt")
gaz_cbsa = get_gaz(["GEOID","NAME","INTPTLAT","INTPTLONG"],"2021_Gaz_cbsa_national.txt")
#####################################################################################
...
headerRef_json = json.load(open("%s%s" % (r"00_resources/","contractHeaderReference.json")))
#####################################################################################
...
#####################################################################################
funds_contract = pd.read_csv(str("%s%s%s" % (r"01_data/",fName_funds_contract,".csv")),encoding="utf-8", low_memory=False)
...
recipientStats_ = {}
#####################################################################################
with open("%s%s" % (r"02_output/","dot_contracts_byCbsa.csv"),'w',newline='', encoding='utf-8') as write_dataOut:
...
write_dataOut.close()
with open(str("%s%s%s%s%s%s" % (r"02_output/",agencyCode,"_",fy,"_","cbsaStats.json")), "w") as json_cbsaStats:
	json_cbsaStats.write(json.dumps(cbsaStats_, indent=2, ensure_ascii=False))

with open(str("%s%s%s%s%s%s" % (r"02_output/",agencyCode,"_",fy,"_","recipientStats.json")), "w") as json_recipientStats:
