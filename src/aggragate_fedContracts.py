...
cbsaStats_json = json.load(open("%s%s" % (r"02_output/fedContracts/","DOE_FY23_cbsaStats.json")))
...
normalized_awardTotals = ((df['AWARDS_TOTAL'] - df['AWARDS_TOTAL'].min()) / (df['AWARDS_TOTAL'].max() - df['AWARDS_TOTAL'].min())+.1)
